...
export async function getActivities() {
  ...
  const userPromises = users.map(async (user) => {
    ...
    skills.filter(skill => skill.id !== "init").forEach(skill => {
      activitiesData.push({
        type: 'skill',
    ...
    projects.filter(project => project.id !== "init").forEach(project => {
      activitiesData.push({
        type: 'project',
    ...
    objectifs.filter(objectif => objectif.id !== "init").forEach(objectif => {
      activitiesData.push({
        type: 'objectif',
